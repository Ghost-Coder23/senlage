...
if (mobileMenu && navLinks) {
   ...
   mobileMenu.addEventListener('click', () => {
      ...
      spans.forEach((span, index) => {
         ...
         span.style.opacity = isActive && index === 1 ? '0' : '';
      });
   });
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   if (headline && paragraph) {
      ...
      const textSets = [{
            h1: "Be Sure!<br>Be safe!",
            p: "SECURITY EQUIPMENT FILTRATION BAGS PROTECTIVE CLOTHING."
         },
         {
            h1: "Your Security<br>Our Priority",
            p: "Because protection is not optional<br> it is essential."
         }
      ];
...
window.addEventListener('scroll', () => {
   if (window.innerWidth <= 940 || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      return;
   }
